...
contractAddress = "0x6a303f9045D4508A6a9aABD8F5921498BbD99b8d"
...
count = 0
file_name = './pythonProject/data/trace3_get.txt'

while True:

    modelList = getModelList()
    idx = np.where(np.array(modelList) == str(count))[0]

    if len(idx) != 0:
        with open(file_name, 'a') as f:
            line = str(count) + '\t' + str(datetime.datetime.now()) + '\n'
            f.write(line)
        print('model {} found'.format(count))
        count += 1
